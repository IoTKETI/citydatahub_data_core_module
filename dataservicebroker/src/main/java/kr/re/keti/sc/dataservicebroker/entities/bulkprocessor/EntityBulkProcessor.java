...
@Component
@Slf4j
public class EntityBulkProcessor<T1 extends CommonEntityFullVO, T2 extends CommonEntityDaoVO>
  implements IBulkProcessor<IngestMessageVO> {
  ...
  /** 에러데이터 기록용 로거 */
  private final Logger errorDataLogger = LoggerFactory.getLogger(
    Constants.KAFKA_REQUEST_ERROR_LOGGER_NAME
  );
  ...
  public Object processBulk(List<IngestMessageVO> requestMessageVOList) {
    ...
    if (!ValidateUtil.isEmptyData(datasetId)) {
      bigDataStorageTypes =
        dataModelManager
          .getDatasetFlowCache(datasetId)
          .getBigDataStorageTypes();
      // 데이터 셋 아이디가 없는 요청 (ex. HTTP) 인 경우 설정에 있는 entity.default.storage 값 기반 동작
    } else {
      BigDataStorageType defaultBigDataStorageType = BigDataStorageType.parseType(
        defaultStorageType
      );
  ...
  private void processNotification(
    List<EntityProcessVO<T1, T2>> entityProcessVOList
  ) {
    ...
    for (EntityProcessVO<T1, T2> entityProcessVO : entityProcessVOList) {
      ...
      if (entityProcessVO.getProcessResultVO().isProcessResult()) {
        ...
        Operation processOperation = entityProcessVO
          .getProcessResultVO()
          .getProcessOperation();
        if (
          processOperation == Operation.CREATE_ENTITY ||
          processOperation == Operation.DELETE_ENTITY
        ) {
          continue;
        }
        ...
        notificationProcessVO.setEntityTypeUri(
          dataModelCacheVO.getDataModelVO().getTypeUri()
        );

        notificationProcessVO.setEntityType(
          entityProcessVO.getEntityFullVO().getType()
        );
        notificationProcessVO.setEventTime(entityDaoVO.getModifiedAt());
        notificationProcessVO.setRequestEntityFullVO(
          entityProcessVO.getEntityFullVO()
        );
   ...
   * @param entityProcessVOList 처리대상VO리스트
   */
  private void storeErrorHistory(
    List<EntityProcessVO<T1, T2>> entityProcessVOList
  ) {
    for (EntityProcessVO<T1, T2> entityProcessVO : entityProcessVOList) {
      // result 가 실패인 경우
  ...
  private void storeErrorHistory(EntityProcessVO<T1, T2> entityProcessVO) {
    ...
    errorLog
      .append("DatasetId=")
      .append(entityProcessVO.getDatasetId())
      .append(", ")
      .append(entityProcessVO.getProcessResultVO().toString())
      .append(", Content=")
      .append(entityProcessVO.getContent());
  ...
  public Object processSingle(IngestMessageVO object) {
    throw new CoreException(
      ...
      "EntityBulkProcessor not support processSingle."
    );
  }
}
	// 데이터셋 아이디 동일 (모두 동일한 토픽에서 얻은 데이터)
