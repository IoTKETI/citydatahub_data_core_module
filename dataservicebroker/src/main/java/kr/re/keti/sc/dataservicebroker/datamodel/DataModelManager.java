...
public class DataModelManager {
    ...
    public List<DataModelCacheVO> getDataModelVOListCache() {    	
    	...
    	return dataModelCache.values().stream().collect(Collectors.toCollection(ArrayList::new));
    }
    ...
    public DataModelCacheVO getDataModelVOCacheByType(String dataModelType) {
    	...
    	if(dataModelType != null && dataModelType.startsWith("http")) {
    		for(DataModelCacheVO dataModelCacheVO : dataModelCache.values()) {
    			if(dataModelCacheVO.getDataModelVO().getTypeUri().equals(dataModelType)) {
    ...
    public DataModelCacheVO getDataModelVOCacheByContext(List<String> context, String dataModelType) {
    	...
    	if(dataModelType != null && dataModelType.startsWith("http")) {
    		for(DataModelCacheVO dataModelCacheVO : dataModelCache.values()) {
    			if(dataModelType.equals(dataModelCacheVO.getDataModelVO().getTypeUri())) {
    				return dataModelCacheVO;
    			}
    		}
    	}
    	...
    	if(contextMap != null) {
    		...
    		if(dataModelTypeFullUri != null) {
    			for(DataModelCacheVO dataModelCacheVO : dataModelCache.values()) {
        			if(dataModelTypeFullUri.equals(dataModelCacheVO.getDataModelVO().getTypeUri())) {
        				return dataModelCacheVO;
        			}
        		}
    		}
    ...
    public void putDataModelCache(DataModelBaseVO dataModelBaseVO) {
		...
		try {
			...
			if(dataModelBaseVO.getStorageMetadata() != null) {
				storageMetadataVO = objectMapper.readValue(dataModelBaseVO.getStorageMetadata(), DataModelStorageMetadataVO.class);
			} else {
				// 하위 호환을 위해 storageMetadata가 DB에 없는 경우 생성
				storageMetadataVO = createDataModelStorageMetadata(dataModelVO, null, dataModelBaseVO.getCreatedStorageTypes());
			}
    ...
    public void putDatasetFlowCache(DatasetFlowBaseVO datasetFlowBase) {
        if (datasetFlowBase != null) {
            log.info("PUT DatasetFlow cache. datasetId={}", datasetFlowBase.getDatasetId());
    ...
    public void putAclRuleCache(AclRuleVO aclRuleVO) {
        if (aclRuleVO != null) {
            log.info("PUT Acl rule cache. id={}", aclRuleVO.getId() + ", datasetId=" + aclRuleVO.getResourceId());
     ...
     * @return
     */
    public DataModelStorageMetadataVO createDataModelStorageMetadata(DataModelVO dataModelVO, DataModelStorageMetadataVO beforeStorageMetadata, List<BigDataStorageType> createdStorageTypes) {

        // 1. dataModel 정보 파싱
    	DataModelStorageMetadataVO storageMetadataVO = new DataModelStorageMetadataVO();

        // 2. 테이블명 생성
        storageMetadataVO.setRdbTableName(generateRdbTableName(dataModelVO.getId()));
        storageMetadataVO.setHiveTableName(generateHiveTableName(dataModelVO.getId()));

        // 3. 추후 DB입력/조회 시 사용될 DbColumnInfoVO 를 생성
        Map<String, DataModelDbColumnVO> beforeDbColumnVOMap = beforeStorageMetadata == null ? null : beforeStorageMetadata.getDbColumnInfoVOMap(); 
        attributesToDbColumnVO(storageMetadataVO.getDbColumnInfoVOMap(), beforeDbColumnVOMap, null, dataModelVO.getAttributes());
     ...
     */
    private void attributesToDbColumnVO(Map<String, DataModelDbColumnVO> dbColumnInfoVOMap, Map<String, DataModelDbColumnVO> beforeDbColumnVOMap, List<String> parentHierarchyIds, List<Attribute> rootAttributes) {
        if (rootAttributes == null) {
            return;
        }

        for (Attribute rootAttribute : rootAttributes) {
            attributesToDbColumnVO(dbColumnInfoVOMap, beforeDbColumnVOMap, parentHierarchyIds, rootAttribute);
        }
    }
     ...
     */
    public void attributesToDbColumnVO(Map<String, DataModelDbColumnVO> dbColumnVOMap, Map<String, DataModelDbColumnVO> beforeDbColumnVOMap,
    		List<String> parentHierarchyIds, Attribute rootAttribute) {
        ...
        if (rootAttribute.getAttributeType() == AttributeType.PROPERTY) {
            ...
            if (rootAttribute.getValueType() == AttributeValueType.OBJECT) {
                ...
                if (objectMembers != null) {
                    objectTypeToCache(currentHierarchyIds, objectMembers, dbColumnVOMap, beforeDbColumnVOMap);
                }
            ...
            // 1-2. value type이 array object인 경우
                ...
                if (objectMembers != null) {
                    for (ObjectMember objectMember : objectMembers) {
                        ...
                        DataModelDbColumnVO dbColumnInfoVO = createDbColumnInfoVO(hierarchyAttributeIds, objectMember.getValueType(),
                                objectMember.getMaxLength(), objectMember.getIsRequired(), true, beforeDbColumnVOMap);
                        dbColumnVOMap.put(dbColumnInfoVO.getColumnName().toLowerCase(), dbColumnInfoVO);
                    }
                }

            // 1-3. value type이 String, Integer, Double, Date, ArrayString, ArrayInteger, ArrayDouble인 경우
            } else {
                DataModelDbColumnVO dbColumnInfoVO = createDbColumnInfoVO(currentHierarchyIds, rootAttribute.getValueType(),
                        rootAttribute.getMaxLength(), rootAttribute.getIsRequired(), false, beforeDbColumnVOMap);
                dbColumnVOMap.put(dbColumnInfoVO.getColumnName().toLowerCase(), dbColumnInfoVO);
            }

        // 2. type이 GepProperty인 경우
        } else if (rootAttribute.getAttributeType() == AttributeType.GEO_PROPERTY) {
            List<DataModelDbColumnVO> dbColumnInfoVOList = createGeoDbColumnInfoVO(currentHierarchyIds, rootAttribute.getIsRequired(), dbColumnVOMap);
        ...
        // 3. type이 Relationship인 경우
            ...
            DataModelDbColumnVO dbColumnInfoVO = createDbColumnInfoVO(currentHierarchyIds, 
            		AttributeValueType.STRING, rootAttribute.getMaxLength(), rootAttribute.getIsRequired(), false, beforeDbColumnVOMap);
            dbColumnVOMap.put(dbColumnInfoVO.getColumnName().toLowerCase(), dbColumnInfoVO);
        }
        ...
        if (rootAttribute.getHasObservedAt() != null && rootAttribute.getHasObservedAt()) {
            ...
            DataModelDbColumnVO dbColumnInfoVO = createDbColumnInfoVO(hierarchyAttributeIds, 
            		AttributeValueType.DATE, null, null, false, beforeDbColumnVOMap);
            dbColumnVOMap.put(dbColumnInfoVO.getColumnName().toLowerCase(), dbColumnInfoVO);
        }

        // 5. has property or relationship 이 존재하는 경우
        if (rootAttribute.getChildAttributes() != null) {
            attributesToDbColumnVO(dbColumnVOMap, beforeDbColumnVOMap, currentHierarchyIds, rootAttribute.getChildAttributes());
        }
        ...
        if (rootAttribute.getHasUnitCode() != null && rootAttribute.getHasUnitCode()) {
            ...
            DataModelDbColumnVO dbColumnInfoVO = createDbColumnInfoVO(hierarchyAttributeIds, 
            		AttributeValueType.STRING, null, null, false, beforeDbColumnVOMap);
            dbColumnVOMap.put(dbColumnInfoVO.getColumnName().toLowerCase(), dbColumnInfoVO);
        }
        
        // 7. createdAt
        List<String> HierarchyCreatedatAttributeIds = new ArrayList<>(currentHierarchyIds);
        HierarchyCreatedatAttributeIds.add(PropertyKey.CREATED_AT.getCode());
        DataModelDbColumnVO createdatDbColumnInfoVO = createDbColumnInfoVO(HierarchyCreatedatAttributeIds, 
        		AttributeValueType.DATE, null, null, false, beforeDbColumnVOMap);
        dbColumnVOMap.put(createdatDbColumnInfoVO.getColumnName().toLowerCase(), createdatDbColumnInfoVO);

   	 	// 8. modifiedAt
        List<String> hierarchyModifiedatAttributeIds = new ArrayList<>(currentHierarchyIds);
        hierarchyModifiedatAttributeIds.add(PropertyKey.MODIFIED_AT.getCode());
        DataModelDbColumnVO modifiedatDbColumnInfoVO = createDbColumnInfoVO(hierarchyModifiedatAttributeIds, 
        		AttributeValueType.DATE, null, null, false, beforeDbColumnVOMap);
        dbColumnVOMap.put(modifiedatDbColumnInfoVO.getColumnName().toLowerCase(), modifiedatDbColumnInfoVO);
    }
     ...
     */
    private void objectTypeToCache(List<String> parentHierarchyIds, List<ObjectMember> objectMembers,
    		Map<String, DataModelDbColumnVO> dbColumnInfoVOMap, Map<String, DataModelDbColumnVO> beforeDbColumnVOMap) {
        ...
        for (ObjectMember objectMember : objectMembers) {
            ...
            if (objectMember.getValueType() == AttributeValueType.OBJECT
                ...
                objectTypeToCache(currentHierarchyIds, objectMember.getObjectMembers(), dbColumnInfoVOMap, beforeDbColumnVOMap);

            } else {

                DataModelDbColumnVO dbColumnInfoVO = createDbColumnInfoVO(currentHierarchyIds, objectMember.getValueType(),
                        objectMember.getMaxLength(), objectMember.getIsRequired(), false, beforeDbColumnVOMap);
                dbColumnInfoVOMap.put(dbColumnInfoVO.getColumnName().toLowerCase(), dbColumnInfoVO);
            }
        }
    }
     ...
     */
    private DataModelDbColumnVO createDbColumnInfoVO(List<String> hierarchyAttributeIds, AttributeValueType valueType,
                                                     String maxLength, Boolean isRequired, boolean isArrayObject,
     ...
     */
    private List<DataModelDbColumnVO> createGeoDbColumnInfoVO(List<String> hierarchyAttributeIds, Boolean isRequired,
        ...
        geo4326ColumnInfoVO.setDaoAttributeId(columnName.toLowerCase() + Constants.GEO_PREFIX_4326);
        geo4326ColumnInfoVO.setColumnName(columnName.toLowerCase() + Constants.GEO_PREFIX_4326);
        ...
        geo3857ColumnInfoVO.setDaoAttributeId(columnName.toLowerCase() + Constants.GEO_PREFIX_3857);
        geo3857ColumnInfoVO.setColumnName(columnName.toLowerCase() + Constants.GEO_PREFIX_3857);
     ...
     * @return DbColumnType
     */
    public DbColumnType valueTypeToDbColumnType(AttributeValueType attributeValueType) {
     ...
     * @return DbColumnType
     */
    public DbColumnType arrayObjectValueTypeToDbColumnType(AttributeValueType attributeValueType) {
        switch (attributeValueType) {
            case STRING:
                return DbColumnType.ARRAY_VARCHAR;
            case INTEGER:
     ...
     */
    public List<String> convertAttrNameToFullUri(List<String> links, List<String> attributeNames) {
    	...
    	for(String contextUri : contextUris) {
    		...
    		if(contextMap == null) {
                ...
                try {
                    ...
                    if(contextVO == null) {
                        throw new NgsiLdContextNotAvailableException(ErrorCode.INVALID_CONTEXT_URI, "Retrieve @context error. body is empty. contextUri=" + contextUri);
                    }
                    ...
                    throw new NgsiLdContextNotAvailableException(ErrorCode.INVALID_CONTEXT_URI, "Retrieve @context error. body is empty. contextUri=" + contextUri, e);
                }
    		}
		...
		if(context instanceof String) {
			if(((String) context).startsWith("http")) {
                ...
                try {
                    ...
                    if(contextVO == null) {
                        throw new NgsiLdContextNotAvailableException(ErrorCode.INVALID_CONTEXT_URI, "Retrieve @context error. body is empty. contextUri=" + (String) context);
                    }
                } catch (IOException e) {
                    throw new NgsiLdContextNotAvailableException(ErrorCode.INVALID_CONTEXT_URI, "Retrieve @context error. body is empty. contextUri=" + (String) context, e);
                }
			...
			throw new NgsiLdContextNotAvailableException(ErrorCode.INVALID_CONTEXT_URI, "Retrieve @context error. Unsupported class type. type=" + context.getClass());
		}
    ...
    private JsonldContextBaseVO getContextByUri(String contextUri) {
		...
		try {
			...
			throw new NgsiLdContextNotAvailableException(ErrorCode.INVALID_CONTEXT_URI, "Retrieve @context error. "
    ...
    public String generateDbColumnName(List<String> hierarchyAttrNames) {
        ...
        if(sb.toString().getBytes().length <= Constants.POSTGRES_COLUMN_NAME_MAX_LENTH) {
        	return sb.toString();
        }
...
//       - ﻿예시 : ﻿transmissivityvolume_﻿XXXX_predictedat_observedat
    	sb.setLength(0);
    	sb.append(hierarchyAttrNames.get(0))
    	  .append(Constants.COLUMN_DELIMITER).append(random4ByteString);

    	if(hierarchyAttrNames.size() > 2 && isSystemPostfixField(hierarchyAttrNames.get(hierarchyAttrNames.size()-1))) {
    		sb.append(Constants.COLUMN_DELIMITER).append(hierarchyAttrNames.get(hierarchyAttrNames.size()-2));
    	}
    	sb.append(Constants.COLUMN_DELIMITER).append(hierarchyAttrNames.get(hierarchyAttrNames.size()-1));
...
//       - ﻿ 예시) ﻿XXXX_observedat
    	...
    	if(isSystemPostfixField(hierarchyAttrNames.get(hierarchyAttrNames.size()-1))) {
    		sb.append(Constants.COLUMN_DELIMITER).append(hierarchyAttrNames.get(hierarchyAttrNames.size()-1));
    	}
    ...
    private boolean isSystemPostfixField(String attributeName) {
    	if(PropertyKey.OBSERVED_AT.getCode().equals(attributeName)
    ...
    public String getColumnNameByStorageMetadata(DataModelStorageMetadataVO storageMetadataVO, List<String> hierarchyAttrNames) {

    	if(storageMetadataVO == null || storageMetadataVO.getDbColumnInfoVOMap() == null || hierarchyAttrNames == null) {
    		return null;
    	}

    	for(DataModelDbColumnVO dataModelDbColumnVO : storageMetadataVO.getDbColumnInfoVOMap().values()) {
    		if(hierarchyAttrNames.equals(dataModelDbColumnVO.getHierarchyAttributeIds())) {
    ...
    public List<String> getColumnNamesByStorageMetadata(DataModelStorageMetadataVO storageMetadataVO, List<String> hierarchyAttrNames) {
