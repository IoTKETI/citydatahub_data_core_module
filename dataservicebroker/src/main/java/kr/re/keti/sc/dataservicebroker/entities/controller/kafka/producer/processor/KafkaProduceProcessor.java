package kr.re.keti.sc.dataservicebroker.entities.controller.kafka.producer.processor;
...
import kr.re.keti.sc.dataservicebroker.entities.controller.kafka.producer.IKafkaProducer;
...
public class KafkaProduceProcessor implements IBulkProcessor<KafkaProcessVO> {
	...
	private void produceToTopic(KafkaProcessVO kafkaProcessVO) {
		...
		try {
			provisionNoti = objectMapper.writeValueAsString(kafkaProcessVO.getNotificationReqeustVO());
		} catch (JsonProcessingException e) {
			String errorMessage = "NotificationReqeustVO Json Parsing ERROR. kafkaProcessVO=" + kafkaProcessVO.toString();
		...
		try {
			...
			String errorMessage = "NotificationReqeustVO Producing ERROR. message=" + new String(sendMessageBytes);
			log.warn(errorMessage, e);
			errorDataLogger.warn(errorMessage, e);
		}
	}
