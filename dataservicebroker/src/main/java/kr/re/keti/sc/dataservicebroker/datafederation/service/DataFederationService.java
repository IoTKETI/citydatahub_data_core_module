...
public class DataFederationService {
    ...
    public DataFederationService(CsourceRegistrationManager csourceRegistrationManager, DataModelDAO dataModelDAO,
                                 EntityRetrieveSVC entityRetrieveSVC, RestTemplate restTemplate,
                                 ObjectMapper objectMapper, DataFederationProperty dataFederationProperty) {
    ...
    public void retrieveAndCachingCsource() {
        ...
        if (registryCsourceRegistrationVOs != null) {
            ...
            log.info("Retrieve and caching csource by Data-Registry. csourceRegistrationVOs={}",
                     registryCsourceRegistrationVOs);
    ...
    public List<CsourceRegistrationVO> getCsourceRegistrations() {
        ...
        String requestUri = dataFederationProperty.getDataRegistry()
                                                  .getBaseUri() + dataFederationProperty.getDataRegistry()
                                                                                        .getSubUri()
                                                                                        .getCsource();
        ...
        ResponseEntity<List<CsourceRegistrationVO>> response = restTemplate.exchange(requestEntity,
                                                                                     new ParameterizedTypeReference<List<CsourceRegistrationVO>>() {
                                                                                     });
    ...
    public ResponseEntity<Void> registerCsource() {
        ...
        String dataRegistryCsourceUri = dataFederationProperty.getDataRegistry()
                                                              .getBaseUri() + dataFederationProperty.getDataRegistry()
                                                                                                    .getSubUri()
                                                                                                    .getCsource();
        try {
            ...
            ResponseEntity<Void> result = restTemplate.exchange(
                    dataRegistryCsourceUri + "/" + csourceRegistrationVO.getId(), HttpMethod.PATCH, entity, Void.class);
            return result;

        }
        catch (HttpClientErrorException e) {
            log.warn("HTTP CsourceRegistration Update Response code={}. Id={}", e.getRawStatusCode(),
                     csourceRegistrationVO.getId(), e);
        }
        catch (Exception e) {
            log.warn("HTTP CsourceRegistration Update error. Id={}", csourceRegistrationVO.getId(), e);
        }
        ...
        try {
            ...
            log.warn("HTTP CsourceRegistration Create Response code={}. Id={}", e.getRawStatusCode(),
                     csourceRegistrationVO.getId(), e);
        }
        catch (Exception e) {
            log.warn("HTTP CsourceRegistration Create error. Id={}", csourceRegistrationVO.getId(), e);
        }
    ...
    public ResponseEntity<Void> registerCsourceSubscription() {
        ...
        // 2-1. 구독 업데이트 요청
        String dataRegistrySubscriptionUri = dataFederationProperty.getDataRegistry()
                                                                   .getBaseUri() + dataFederationProperty.getDataRegistry()
                                                                                                         .getSubUri()
                                                                                                         .getSubscription();
        try {
            ...
            ResponseEntity<Void> result = restTemplate.exchange(
                    dataRegistrySubscriptionUri + "/" + subscriptionVO.getId(), HttpMethod.PATCH, entity, Void.class);
            return result;
        }
        catch (HttpClientErrorException e) {
            log.warn("HTTP CsourceRegistration Update Response code={}. Id={}", e.getRawStatusCode(),
            ...
            log.warn("HTTP CsourceRegistration Update error. Id={}", subscriptionVO.getId(), e);
        }
    ...
    public CsourceRegistrationVO createCsourceRegistrationInfo() {
        ...
        csourceRegistrationVO.setInformation(
                generateInformation(dataModelBaseVOs, dataFederationProperty.getCsource().getEntityInfos()));
    ...
    private List<Information> generateInformation(List<DataModelBaseVO> dataModelBaseVOs,
        ...
        for (DataModelBaseVO dataModelBaseVO : dataModelBaseVOs) {
            try {
                ...
                log.warn("generateInformation warn. e={}, model typeUri={}", e.toString(),
                         dataModelBaseVO.getTypeUri());
            }
            catch (Exception e) {
                log.warn("createCsourceRegistrationInfo error. invalid Model. dataModel={}",
                         dataModelBaseVO.getDataModel(), e);
            }
        }
        return informations;
    }
    ...
    private Information dataModelToInformation(DataModelVO dataModel) {
        ...
        if (!ValidateUtil.isEmptyData(idPattern)) {
            ...
            EntityRetrieveVO entityRetrieveVO = entityRetrieveSVC.getEntityStandalone(queryVO, null,
