...
import org.apache.spark.sql.hive.thriftserver.GeohikerThriftServer.{GeohikerExecutionInfo, GeohikerSessionInfo}
import org.apache.spark.ui._
import org.apache.spark.ui.UIUtils._


/** Page for Spark Web UI that shows statistics of the thrift server */
private[ui] class GeohikerThriftServerPage(parent: GeohikerThriftServerTab) extends WebUIPage("") with Logging {
  ...
  private def generateSQLStatsTable(request: HttpServletRequest): Seq[Node] = {
    ...
    val table = if (numStatement > 0) {
      ...
      def generateDataRow(info: GeohikerExecutionInfo): Seq[Node] = {
  ...
  private def generateSessionStatsTable(request: HttpServletRequest): Seq[Node] = {
    ...
    val table = if (numBatches > 0) {
      ...
      val headerRow = Seq("User", "IP", "Session ID", "Start Time", "Finish Time", "Duration",
        "Total Execute")
      def generateDataRow(session: GeohikerSessionInfo): Seq[Node] = {
