 ...
 */
private[hive] object GeohikerSparkSQLCLIDriver extends Logging {
    ...
    HiveInterruptUtils.add(new HiveInterruptCallback() {
      override def interrupt(): Unit = {
        ...
        if (SparkSQLEnv.sparkContext != null) {
          GeohikerSparkSQLEnv.sparkContext.cancelAllJobs()
        } else {
          if (transport != null) {
  ...
  def main(args: Array[String]): Unit = {
    ...
    ShutdownHookManager.addShutdownHook { () => GeohikerSparkSQLEnv.stop() }
...
private[hive] class GeohikerSparkSQLCLIDriver extends CliDriver with Logging {
  ...
  private val isRemoteMode = {
    GeohikerSparkSQLCLIDriver.isRemoteMode(sessionState)
  }

  private val conf: Configuration =
    if (sessionState != null) sessionState.getConf else new Configuration()

  // Force initializing GeohikerSparkSQLEnv. This is put here but not object SparkSQLCliDriver
  // because the Hive unit tests do not go through the main() code path.
  if (!isRemoteMode) {
    GeohikerSparkSQLEnv.init()
    if (sessionState.getIsSilent) {
      GeohikerSparkSQLEnv.sparkContext.setLogLevel(Level.WARN.toString)
    }
  } else {
    // Hive 1.2 + not supported in CLI
    throw new RuntimeException("Remote operations not supported")
  ...
  def printMasterAndAppId(): Unit = {
    val master = GeohikerSparkSQLEnv.sparkContext.master
    val appId = GeohikerSparkSQLEnv.sparkContext.applicationId
