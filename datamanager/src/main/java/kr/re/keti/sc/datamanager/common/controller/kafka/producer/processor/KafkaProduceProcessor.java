package kr.re.keti.sc.datamanager.common.controller.kafka.producer.processor;
...
import kr.re.keti.sc.datamanager.common.bulk.IBulkProcessor;
...
import kr.re.keti.sc.datamanager.common.controller.kafka.producer.IKafkaProducer;
...
public class KafkaProduceProcessor implements IBulkProcessor<KafkaProcessVO> {
	 ...
	 * @param kafkaProcessVO 카프카 데이터 전송 처리 VO
	 */
	private void produceToTopic(KafkaProcessVO kafkaProcessVO) {
		
		// 1. 데이터 파싱
		String provisionNoti = null;
		try {
			provisionNoti = objectMapper.writeValueAsString(kafkaProcessVO.getProvisionNotiVO());
		} catch (JsonProcessingException e) {
			String errorMessage = "provisionNoti Json Parsing ERROR. kafkaProcessVO=" + kafkaProcessVO.toString();
		...
		byte[] sendMessageBytes = createSendMessage(provisionNoti.getBytes());

		// 3. 데이터 전송
		try {
			kafkaProducer.send(kafkaProcessVO.getTopic(), sendMessageBytes);
		} catch (Exception e) {
	...
	public void setEntityChangeEventProducer(IKafkaProducer kafkaProducer) {
